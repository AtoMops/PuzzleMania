...
public class PuzzleManiaAppV8 extends Application {
...
//		rect.setStrokeWidth(5.0);
	...
	private void checkManaConstellation() {
		...
		/*
		 * wir brauchen NUR die ID's der ImageViews dann können wir testen ob 3 oder
		 * mehr in Reihe sind oder nicht also einfach durchiterieren sollte reichen jede
		 * Kolumne und jede Row in einen Array den wir testen
		 * 
		 * --> die Dinge so einfach halten wie möglich ^^'
		 */

		/*
		 * String newID = "#"+ rowPos+colPos; // HBox auf cursor-Position HBox
		 * hBoxAtCursorPos = (HBox) gamePane.lookup(cursorPosId);
		 * 
		 * 
		 */

		List<List<String>> lstRowLists = new ArrayList<List<String>>();
		for (int i = 0; i < 8; i++) {
			...
			// wir lesen erst ROWS aus (also von links nach rechts)
			lstRowLists.add(lstRow);
		}

		// für die Cols einf row-Liste invertieren
		List<List<String>> lstColLists = new ArrayList<List<String>>();
		for (int i = 0; i < lstRowLists.size(); i++) {
			List<String> lstCol = new ArrayList<>();
			for (int j = 0; j < lstRowLists.size(); j++) {
				lstCol.add(lstRowLists.get(j).get(i));
			}
			lstColLists.add(lstCol);
		}
		...
		for (List<String> list : lstRowLists) {
			...
			List<List<Integer>> chkmatchListRow = checkForManaMatches(list);
		...
		for (List<String> list : lstColLists) {
			System.out.println("Row check: " + k);
			List<List<Integer>> chkmatchListCol = checkForManaMatches(list);
		...
		// hier passende Rows löschen geht (nachrücken fehlt --> erstmal NUR logik! Effekte später)
		for (String string : idMatchesRows) {
		...
		// hier noch cols
		for (String string : idMatchesCols) {
			HBox currHBox = (HBox) gamePane.lookup(string);
			ImageView imgCurr = (ImageView) currHBox.getChildren().get(0);
			imgCurr.setImage(null);
		}
	...
	private List<List<Integer>> checkForManaMatches(List<String> inList) {
		...
		// here check if Mana appears continuous > 2 
		for (int j = 0; j < opt.size(); j++) {
			List<Integer> intList = new ArrayList<Integer>();
			for (int i = 0; i < inList.size(); i++) {
				if (inList.get(i).equals(opt.get(j))) {
					intList.add(i);
				}
			}
			...
			// letzte position adden
			if (intList2.size() >=2 ) {
				intList2.add(intList2.get(intList2.size()-1)+1);
				// wenn matches gefunden dann speichern (mehr als 2 pro Reihe sind nicht zu erwarten)
				allLineMatches.add(intList2);
			} 
		}
