...
import javafx.util.Duration;
	...
	/*
	 * Switch zwischen Panes auf Root geht soweit
	 * Sound ist auch ok 
	 * 
	 * Jetzt mal Symbole in das GameBoard packen (mit Enum)
	 * Symbole sollen markiert und dann verschoben werden können
	 * verschieben nur um +1 von der aktuellen Position!
	 * 
	 * 
	 * auch das GameBoard mal hübscher
	 * wir brauchen noch ein Back-Btn zum hauptmenü oder so ein sub-menü das man auf key öffnen kann
	 *  
	 *  Aber ZUERST!! --> einen Cursor der vernünftig funtioniert ^^' 
	 * Also auch eine Resonanz wenn aktivierkt wird
	 * --> erstmal funktional; hübsch machen wir den später
	 * 
	 * siehe hier:
	 * 	getCursorPosRowCol()
	 * 
	 * -> die ganze cursor-bewegung ist hier ziemlich unübersichtlich geproggt
	 *  wir müssen das mal zusammenpacken in eine vernünftige methode oO'
	 * 
	 */

public class PuzzleManiaAppV4 extends Application{
...
//			gamePane.getRowConstraints().add(new RowConstraints(5));
			...
			/* hier verwenden wir "createRectangleForTesting"
			 * Diese mit ImageViews austauschen?!
			 * wir müssen die später auf dem Board verschieben können bzw soll das so aussehen
			 * 
			 * 
			 */
...
//					gamePane.add(createRectangleForTesting(), j,i); // gamePane uses add(Node,x,y)
		...
		/* 
		   der Cursor soll frei verschiebar sein und eine Resonanz geben
		   wenn SPACE gedrpckt wird
		   die Resonanz soll mit SPACE auch wieder deaktiert werden können
...
//					System.out.println("id: " + id);
		...
		private void setCursorMarker(String id) {
			...
			/* id ist die Cursor-Position auf der Cursor-Pane (Position: 00-77)
			 * wir erstellen kein neues Rectangle sondern ändern das Rectangle
			 * an der Position die makiert werden soll
			 * --> das "sieht dann so aus" als ob da ein cursor wäre
			 *   
			 */
...
//						if (rec.getFill() == Color.DARKORANGE) {
						if (rec.getStroke() != null) { // nur der Cursor hat stroke (border)
							System.out.println("current cursorPos is: " + rec.getId());
			...
			switch (direction) {
				case "UP":
					if (rowPos > 0) {
						...
						if (rectOld != null) {
							rectOld.setFill(null);
							rectOld.setStroke(null);
						}else {
							System.out.println("is null");
						}
		...
		private Rectangle changeCursorApperance(Rectangle rect) {
			double borderSize = 5;
			rect.setHeight(rect.getHeight()-borderSize);
			rect.setWidth(rect.getWidth()-borderSize);
			rect.setFill(null);
