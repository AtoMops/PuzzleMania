...
public class PuzzleManiaAppV15 extends Application {
...
//		playerPane = new Pane();
		 ...
		 * 
		 * Wir brauchen 4 Mana-ProgressBars
		 * 
		 * Diesmal brauchen wir keinen Slider weil wir die Daten ja ohne User-Interktion übergeben
		 * ABER wir brauchen wahrscheinlich schon einen CSS-Style
		 * Die Bar am besten in einen Container Packen (HBox oder VBox)
		 * 
		 * Wenn wir wissen wie das so aussehen sollen vlt besser ein ManaVar-Objekt erstellen
		 * das ProgressBar extended
		 * 
		 * so wie hier:
		 ...
		 * 
		 * --> da hat auch einer eine custom-PB aus 2 VBoxes gebastelt oO' 
		 * 		könnte auch gehen..
		 *
		 ...
		 *
		 * 
		 * --> ganz ok jetzt noch die andere Seite
		 * 		
		 */
		
		List<ProgressBar> lstManaBars = new ArrayList<ProgressBar>();
		for (int i = 0; i < 4; i++) {
			lstManaBars.add(createManaBar(i));
		}
...
//		hBoxManaBars.setPadding(new Insets(10, 10, 10, 10));
		...
		Rectangle manaBoxFrame = createFrameForPlayerMenu(inlayWidth,
														  100);
...
//		hBoxManaBars.setPadding(new Insets(10, 10, 10, 10));
	...
	private ProgressBar createManaBar(int manaColor) {
...
//	                new Translate(0, 0),
		...
		switch (manaColor) {
		case 0:
			pb.setStyle("-fx-base: white; -fx-accent: red;");
			pb.setId("pbRed");
			break;
		case 1:
			pb.setStyle("-fx-base: white; -fx-accent: green;");
			pb.setId("pbGreen");
			break;
		case 2:
			pb.setStyle("-fx-base: white; -fx-accent: yellow;");
			pb.setId("pbYellow");
			break;
		case 3:
			pb.setStyle("-fx-base: white; -fx-accent: blue;");
			pb.setId("pbBlue");
			break;
		default:
			break;
